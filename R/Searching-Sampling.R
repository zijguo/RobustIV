...
#' \item{\code{VHat.init}}{a numeric vector denoting the initially estimated set of valid and relevant IVs.}
...
#' \item{\code{rule.clique}}{a boolean matrix where each row represents whether the identification condition is satisfied or not. Only returns when \code{voting} is \code{'MaxClique'}.}
#' \item{\code{max.cliques}}{a numeric matrix where each row represents each maximum clique of initial. Only returns when \code{voting} is \code{'MaxClique'}.}
#' @export
#'
#' @examples
#' \dontrun{
...
#' }
Searching.Sampling <- function(Y, D, Z, X, intercept = TRUE, alpha = 0.05, alpha0 = 0.01, boot.SHat = TRUE,tuning = 2.01, method = "OLS",
                               voting = 'MaxClique', Sampling=TRUE, boot.value=TRUE, M = 1000){
  ...
  stopifnot(is.numeric(alpha0),length(alpha0) == 1,alpha0 <= 1,alpha0 >= 0)
  ...
  if (method == 'OLS') {
    inputs = TSHT.OLS(Y,D,W,pz,intercept=intercept)
  }else if (method == "DeLasso") {
    inputs = TSHT.DeLasso(Y,D,W,pz,intercept=intercept)
  }

  # Compute covariance of W and W %*% U
  covW = inputs$covW  #this should automatically turn covW into a matrix
  ...
  SigmaYD = inputs$SigmaYD #s um(qr.resid(qrW,Y) * qr.resid(qrW,D)) / (n - p-1)
  ...
  if (voting == 'MaxClique') {
    ...
    for (i in 1:length(max.clique)) {
      V0.hat <- sort(as.numeric(max.clique[[i]]))
...
#' ### TSHT.Initial (This modifies the original TSHT function and produces an initial estimator for sampling and searching)
...
#'                          boot.SHat=FALSE,tuning = 2.01,max_clique=FALSE) {
